...
public class OutletSessionBean implements OutletSessionBeanRemote, OutletSessionBeanLocal {
    ...
    public void checkOutletAvailability(Date date, Long outletId) throws OutsideOutletAvailability{
        ...
        if(outlet.getOpeningHours() != null) {
            ...
            System.out.println(Integer.parseInt(outlet.getOpeningHours().substring(0, 2)) * 60 + Integer.parseInt(outlet.getOpeningHours().substring(3,5)));
            System.out.println(cal.get(Calendar.HOUR_OF_DAY)*60 + cal.get(Calendar.MINUTE));
            System.out.println(Integer.parseInt(outlet.getClosingHours().substring(0, 2)) * 60 + Integer.parseInt(outlet.getClosingHours().substring(3,5)));
            
            if(cal.get(Calendar.HOUR_OF_DAY)*60 + cal.get(Calendar.MINUTE) < Integer.parseInt(outlet.getOpeningHours().substring(0, 2)) * 60 + Integer.parseInt(outlet.getOpeningHours().substring(3,5)) || cal.get(Calendar.HOUR_OF_DAY)*60 + cal.get(Calendar.MINUTE) > Integer.parseInt(outlet.getClosingHours().substring(0, 2)) * 60 + Integer.parseInt(outlet.getClosingHours().substring(3,5))) {
